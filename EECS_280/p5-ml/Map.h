  ...
  class PairComp {
    ...
    public:
      bool operator()(const Pair_type &pair1, const Pair_type &pair2) const{
        return key_less(pair1.first, pair2.first);
      }
  };
...
public:
  ...
  Iterator find(const Key_type& k) const{
    Pair_type p1;
    p1.first = k;
    p1.second = Value_type();
    return bst_private.find(p1);
  }
  ...
  Value_type& operator[](const Key_type& k){
    if (find(k) != bst_private.end()){
      ...
      p1.first = k;
      p1.second = Value_type();
      return (bst_private.insert(p1))->second;
    }
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    ...
    if (bst_private.find(val) == bst_private.end()){
      p1.first = bst_private.insert(val);
      p1.second = false;
      return p1;
    }
    else{
      p1.first = bst_private.find(val);
      p1.second = true;
      return p1;
    }
  }
  ...
  Iterator begin() const{
    return bst_private.begin();
  }

  // EFFECTS : Returns an iterator to "past-the-end".
  Iterator end() const{
    return bst_private.end();
  }

private:
  // Add a BinarySearchTree private member HERE.
  BinarySearchTree<Pair_type, PairComp> bst_private;
};
