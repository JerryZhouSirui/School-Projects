...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    ...
    int left_side = height_impl(node->left);
    int right_side = height_impl(node->right);
  ...
  static Node *copy_nodes_impl(Node *node) {
    if(empty_impl(node)) {
      return nullptr; //or mayble nullptr, not sure though
    }
    
    Node *nodes = new Node;
    nodes->datum = node->datum;

    nodes->left = copy_nodes_impl(node->left);
    nodes->right = copy_nodes_impl(node->right);
    return nodes;
  }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    else if(!less(query, node->datum) && !less(node->datum, query)) {
      return node;
    }
  ...
  static Node * max_element_impl(Node *node) {
    ...
    else return (max_element_impl(node->right));
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
   if (empty_impl(node)){
      ...
      else if (less(max_element_impl(node->left)->datum, node->datum)) {
        left_tree = true;
      }
      else {
        left_tree = false;
      ...
      else if (less(node->datum, (min_element_impl(node->right))->datum)) {
        right_tree = true;
      }
      else {
        right_tree = false;
      }
      return left_tree && right_tree && check_sorting_invariant_impl(node->left, less)
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
   if(empty_impl(node)) {
    ...
    else if (!less(val, node->datum) && node->right == nullptr) {
      return nullptr;
    }
